
# Results vs. 3.12.0

- fork: python
- ref: 1aec0644447e69e981d5
- machine: windows-amd64
- commit hash: 1aec064
- commit date: 2024-02-01
- overall geometric mean: 1.02x faster \*
- HPT reliability: 99.75%
- HPT 99th percentile: 1.00x faster
- Memory change: unknown

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240201-pythonperf1-amd64-python-1aec0644447e69e981d5-3.13.0a3+-1aec064 |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| 2to3           | 218 ms                                                      | 219 ms: 1.01x slower                                                        |
| chameleon      | 4.98 ms                                                     | 4.89 ms: 1.02x faster                                                       |
| docutils       | 1.66 sec                                                    | 1.62 sec: 1.03x faster                                                      |
| tornado_http   | 89.5 ms                                                     | 86.9 ms: 1.03x faster                                                       |
| Geometric mean | (ref)                                                       | 1.02x faster                                                                |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240201-pythonperf1-amd64-python-1aec0644447e69e981d5-3.13.0a3+-1aec064 |
|----------------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| async_tree_cpu_io_mixed    | 489 ms                                                      | 450 ms: 1.09x faster                                                        |
| async_tree_none            | 291 ms                                                      | 272 ms: 1.07x faster                                                        |
| async_tree_cpu_io_mixed_tg | 502 ms                                                      | 470 ms: 1.07x faster                                                        |
| async_tree_memoization_tg  | 367 ms                                                      | 347 ms: 1.06x faster                                                        |
| async_tree_none_tg         | 285 ms                                                      | 277 ms: 1.03x faster                                                        |
| async_tree_io_tg           | 771 ms                                                      | 755 ms: 1.02x faster                                                        |
| async_tree_io              | 731 ms                                                      | 721 ms: 1.01x faster                                                        |
| Geometric mean             | (ref)                                                       | 1.04x faster                                                                |

Benchmark hidden because not significant (1): async_tree_memoization

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240201-pythonperf1-amd64-python-1aec0644447e69e981d5-3.13.0a3+-1aec064 |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| nbody          | 71.9 ms                                                     | 61.0 ms: 1.18x faster                                                       |
| float          | 56.8 ms                                                     | 50.4 ms: 1.13x faster                                                       |
| pidigits       | 152 ms                                                      | 153 ms: 1.00x slower                                                        |
| Geometric mean | (ref)                                                       | 1.10x faster                                                                |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240201-pythonperf1-amd64-python-1aec0644447e69e981d5-3.13.0a3+-1aec064 |
|----------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| regex_compile  | 87.5 ms                                                     | 81.0 ms: 1.08x faster                                                       |
| regex_dna      | 126 ms                                                      | 122 ms: 1.04x faster                                                        |
| regex_effbot   | 1.62 ms                                                     | 1.57 ms: 1.03x faster                                                       |
| regex_v8       | 14.2 ms                                                     | 21.8 ms: 1.53x slower                                                       |
| Geometric mean | (ref)                                                       | 1.07x slower                                                                |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240201-pythonperf1-amd64-python-1aec0644447e69e981d5-3.13.0a3+-1aec064 |
|----------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| pickle_pure_python   | 195 us                                                      | 179 us: 1.09x faster                                                        |
| tomli_loads          | 1.37 sec                                                    | 1.30 sec: 1.05x faster                                                      |
| unpickle_pure_python | 133 us                                                      | 127 us: 1.05x faster                                                        |
| json_dumps           | 5.70 ms                                                     | 5.48 ms: 1.04x faster                                                       |
| xml_etree_generate   | 55.8 ms                                                     | 53.8 ms: 1.04x faster                                                       |
| xml_etree_process    | 37.7 ms                                                     | 36.9 ms: 1.02x faster                                                       |
| xml_etree_iterparse  | 65.2 ms                                                     | 63.8 ms: 1.02x faster                                                       |
| json_loads           | 13.9 us                                                     | 13.6 us: 1.02x faster                                                       |
| pickle_dict          | 18.4 us                                                     | 18.1 us: 1.02x faster                                                       |
| pickle               | 7.43 us                                                     | 7.48 us: 1.01x slower                                                       |
| xml_etree_parse      | 93.0 ms                                                     | 93.8 ms: 1.01x slower                                                       |
| unpickle             | 8.18 us                                                     | 8.81 us: 1.08x slower                                                       |
| Geometric mean       | (ref)                                                       | 1.02x faster                                                                |

Benchmark hidden because not significant (2): unpickle_list, pickle_list

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240201-pythonperf1-amd64-python-1aec0644447e69e981d5-3.13.0a3+-1aec064 |
|------------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| python_startup         | 19.5 ms                                                     | 20.9 ms: 1.07x slower                                                       |
| python_startup_no_site | 16.2 ms                                                     | 18.9 ms: 1.16x slower                                                       |
| Geometric mean         | (ref)                                                       | 1.12x slower                                                                |

Benchmarks with tag 'template':
===============================

| Benchmark | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240201-pythonperf1-amd64-python-1aec0644447e69e981d5-3.13.0a3+-1aec064 |
|-----------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| mako      | 7.09 ms                                                     | 6.79 ms: 1.04x faster                                                       |

All benchmarks:
===============

| Benchmark                  | bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0 | bm-20240201-pythonperf1-amd64-python-1aec0644447e69e981d5-3.13.0a3+-1aec064 |
|----------------------------|:-----------------------------------------------------------:|:---------------------------------------------------------------------------:|
| typing_runtime_protocols   | 95.1 us                                                     | 70.9 us: 1.34x faster                                                       |
| comprehensions             | 14.1 us                                                     | 11.5 us: 1.22x faster                                                       |
| nbody                      | 71.9 ms                                                     | 61.0 ms: 1.18x faster                                                       |
| mypy2                      | 509 ms                                                      | 439 ms: 1.16x faster                                                        |
| asyncio_tcp_ssl            | 2.10 sec                                                    | 1.84 sec: 1.14x faster                                                      |
| sqlite_synth               | 1.76 us                                                     | 1.54 us: 1.14x faster                                                       |
| generators                 | 22.5 ms                                                     | 19.9 ms: 1.13x faster                                                       |
| float                      | 56.8 ms                                                     | 50.4 ms: 1.13x faster                                                       |
| logging_silent             | 60.5 ns                                                     | 53.9 ns: 1.12x faster                                                       |
| raytrace                   | 192 ms                                                      | 171 ms: 1.12x faster                                                        |
| richards                   | 28.4 ms                                                     | 25.9 ms: 1.10x faster                                                       |
| pickle_pure_python         | 195 us                                                      | 179 us: 1.09x faster                                                        |
| coroutines                 | 14.3 ms                                                     | 13.0 ms: 1.09x faster                                                       |
| nqueens                    | 62.8 ms                                                     | 57.5 ms: 1.09x faster                                                       |
| async_tree_cpu_io_mixed    | 489 ms                                                      | 450 ms: 1.09x faster                                                        |
| richards_super             | 32.1 ms                                                     | 29.6 ms: 1.08x faster                                                       |
| regex_compile              | 87.5 ms                                                     | 81.0 ms: 1.08x faster                                                       |
| async_tree_none            | 291 ms                                                      | 272 ms: 1.07x faster                                                        |
| deepcopy_reduce            | 2.09 us                                                     | 1.96 us: 1.07x faster                                                       |
| crypto_pyaes               | 48.5 ms                                                     | 45.3 ms: 1.07x faster                                                       |
| async_tree_cpu_io_mixed_tg | 502 ms                                                      | 470 ms: 1.07x faster                                                        |
| async_tree_memoization_tg  | 367 ms                                                      | 347 ms: 1.06x faster                                                        |
| json                       | 3.05 ms                                                     | 2.89 ms: 1.06x faster                                                       |
| sqlglot_parse              | 804 us                                                      | 764 us: 1.05x faster                                                        |
| deepcopy                   | 238 us                                                      | 226 us: 1.05x faster                                                        |
| tomli_loads                | 1.37 sec                                                    | 1.30 sec: 1.05x faster                                                      |
| deltablue                  | 2.16 ms                                                     | 2.06 ms: 1.05x faster                                                       |
| logging_simple             | 6.28 us                                                     | 5.98 us: 1.05x faster                                                       |
| deepcopy_memo              | 23.7 us                                                     | 22.6 us: 1.05x faster                                                       |
| unpickle_pure_python       | 133 us                                                      | 127 us: 1.05x faster                                                        |
| sqlglot_transpile          | 1.02 ms                                                     | 977 us: 1.05x faster                                                        |
| logging_format             | 6.72 us                                                     | 6.43 us: 1.04x faster                                                       |
| mako                       | 7.09 ms                                                     | 6.79 ms: 1.04x faster                                                       |
| dulwich_log                | 44.3 ms                                                     | 42.6 ms: 1.04x faster                                                       |
| json_dumps                 | 5.70 ms                                                     | 5.48 ms: 1.04x faster                                                       |
| xml_etree_generate         | 55.8 ms                                                     | 53.8 ms: 1.04x faster                                                       |
| regex_dna                  | 126 ms                                                      | 122 ms: 1.04x faster                                                        |
| sympy_str                  | 175 ms                                                      | 169 ms: 1.04x faster                                                        |
| tornado_http               | 89.5 ms                                                     | 86.9 ms: 1.03x faster                                                       |
| scimark_lu                 | 58.9 ms                                                     | 57.2 ms: 1.03x faster                                                       |
| async_tree_none_tg         | 285 ms                                                      | 277 ms: 1.03x faster                                                        |
| regex_effbot               | 1.62 ms                                                     | 1.57 ms: 1.03x faster                                                       |
| docutils                   | 1.66 sec                                                    | 1.62 sec: 1.03x faster                                                      |
| pprint_safe_repr           | 513 ms                                                      | 501 ms: 1.02x faster                                                        |
| xml_etree_process          | 37.7 ms                                                     | 36.9 ms: 1.02x faster                                                       |
| xml_etree_iterparse        | 65.2 ms                                                     | 63.8 ms: 1.02x faster                                                       |
| sqlglot_normalize          | 187 ms                                                      | 183 ms: 1.02x faster                                                        |
| async_tree_io_tg           | 771 ms                                                      | 755 ms: 1.02x faster                                                        |
| json_loads                 | 13.9 us                                                     | 13.6 us: 1.02x faster                                                       |
| chameleon                  | 4.98 ms                                                     | 4.89 ms: 1.02x faster                                                       |
| spectral_norm              | 66.9 ms                                                     | 65.8 ms: 1.02x faster                                                       |
| chaos                      | 43.3 ms                                                     | 42.6 ms: 1.02x faster                                                       |
| pickle_dict                | 18.4 us                                                     | 18.1 us: 1.02x faster                                                       |
| async_tree_io              | 731 ms                                                      | 721 ms: 1.01x faster                                                        |
| create_gc_cycles           | 752 us                                                      | 742 us: 1.01x faster                                                        |
| fannkuch                   | 247 ms                                                      | 244 ms: 1.01x faster                                                        |
| scimark_sor                | 78.8 ms                                                     | 77.9 ms: 1.01x faster                                                       |
| sympy_sum                  | 91.5 ms                                                     | 91.0 ms: 1.01x faster                                                       |
| pidigits                   | 152 ms                                                      | 153 ms: 1.00x slower                                                        |
| 2to3                       | 218 ms                                                      | 219 ms: 1.01x slower                                                        |
| pickle                     | 7.43 us                                                     | 7.48 us: 1.01x slower                                                       |
| xml_etree_parse            | 93.0 ms                                                     | 93.8 ms: 1.01x slower                                                       |
| pathlib                    | 80.5 ms                                                     | 81.5 ms: 1.01x slower                                                       |
| bench_mp_pool              | 69.2 ms                                                     | 70.2 ms: 1.02x slower                                                       |
| dask                       | 263 ms                                                      | 268 ms: 1.02x slower                                                        |
| sqlglot_optimize           | 34.5 ms                                                     | 35.2 ms: 1.02x slower                                                       |
| meteor_contest             | 74.6 ms                                                     | 76.2 ms: 1.02x slower                                                       |
| bench_thread_pool          | 857 us                                                      | 876 us: 1.02x slower                                                        |
| unpack_sequence            | 37.5 ns                                                     | 38.7 ns: 1.03x slower                                                       |
| sympy_integrate            | 13.0 ms                                                     | 13.4 ms: 1.03x slower                                                       |
| scimark_fft                | 184 ms                                                      | 195 ms: 1.06x slower                                                        |
| scimark_sparse_mat_mult    | 2.56 ms                                                     | 2.73 ms: 1.07x slower                                                       |
| pyflate                    | 295 ms                                                      | 315 ms: 1.07x slower                                                        |
| python_startup             | 19.5 ms                                                     | 20.9 ms: 1.07x slower                                                       |
| unpickle                   | 8.18 us                                                     | 8.81 us: 1.08x slower                                                       |
| telco                      | 4.13 ms                                                     | 4.46 ms: 1.08x slower                                                       |
| go                         | 91.6 ms                                                     | 99.3 ms: 1.08x slower                                                       |
| python_startup_no_site     | 16.2 ms                                                     | 18.9 ms: 1.16x slower                                                       |
| coverage                   | 40.8 ms                                                     | 47.6 ms: 1.17x slower                                                       |
| mdp                        | 1.37 sec                                                    | 1.68 sec: 1.22x slower                                                      |
| hexiom                     | 4.10 ms                                                     | 5.27 ms: 1.29x slower                                                       |
| scimark_monte_carlo        | 43.7 ms                                                     | 57.0 ms: 1.30x slower                                                       |
| regex_v8                   | 14.2 ms                                                     | 21.8 ms: 1.53x slower                                                       |
| Geometric mean             | (ref)                                                       | 1.02x faster                                                                |

Benchmark hidden because not significant (9): pprint_pformat, gc_traversal, pycparser, unpickle_list, pickle_list, sympy_expand, async_generators, async_tree_memoization, asyncio_tcp
Ignored benchmarks (4) of results/bm-20231002-3.12.0-0fb18b0/bm-20231002-pythonperf1-amd64-python-v3.12.0-3.12.0-0fb18b0.json: aiohttp, django_template, sqlalchemy_declarative, sqlalchemy_imperative


# HPT report

- Reliability score: 99.75% likely to be faster
- 90% likely to have a speedup of 1.01x
- 95% likely to have a speedup of 1.00x
- 99% likely to have a speedup of 1.00x


# Memory

- memory change: unknown